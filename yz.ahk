...
Gui,2: Show,NA
hwnd1 := WinExist()
pBitmap1 := Gdip_CreateBitmapFromFile("code.png")
if !pBitmap1
{
	MsgBox, 48, File loading error!, Could not load 'code.png'
	ExitApp
}
...
TRAYMENU:
	...
	Menu,TRAY,Add,输入法1.0（&Y）,LoadLogo
	Menu,TRAY, Icon,输入法1.0（&Y）, Chinese.ico
	Menu,TRAY,Add
	...
	Menu,TRAY,Default,输入法1.0（&Y）
	Menu,Tray,Tip,极光
	Menu,TRAY,Icon, Chinese.ico
	Menu,Tray,Click,1
return
...
LoadLogo:
	...
	Gui, 1: Show, CEnter NA, 极光
	hwnd1 := WinExist()
	pBitmap := Gdip_CreateBitmapFromFile("code.png")
	if !pBitmap
	{
		MsgBox, 48, File loading error!, Could not load 'code.png'
		ExitApp
...
IME_Return_0E1C(WinTitle="A"){			;获取输入法状态的内容,减少了不必要的切换,切换更流畅了
